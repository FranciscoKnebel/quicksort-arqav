...
void FORCE_INLINE align_masks(uint8_t& a, uint8_t& b, uint8_t& rem_a, uint8_t& rem_b, __m256i& shuffle_a, __m256i& shuffle_b) {
  ...
  int __attribute__((__aligned__(32))) tmpshufa[8];
  int __attribute__((__aligned__(32))) tmpshufb[8];
...
void FORCE_INLINE partition_epi32(int* array, int pv, int* left, int* right) {
...
void quicksort_avx2(int* array, int left, int right) {
  int i = left;
  int j = right;
  ...
  if (left < j) {
    quicksort_avx2(array, left, j);
  }

  if (i < right) {
    quicksort_avx2(array, i, right);
  }
}
